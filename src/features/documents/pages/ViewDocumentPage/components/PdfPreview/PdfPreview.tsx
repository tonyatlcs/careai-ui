...
export const PdfPreview: FC<PdfPreviewProps> = ({
  ...
  useLayoutEffect(() => {
    ...
    if (!canvas || !pdfDoc || content.documentKind !== "pdf" || !originalReady) {
      return;
    }
    let cancelled = false;
    void (async () => {
      ...
      const hasPageGeometry =
        ...
        EXTRACTION_FIELD_KEYS.some((k) => content.fieldBoxes[k].some((bx) => bx.page === selectedPage));
      /**
       ...
       */
      const alignScale =
        content.boxesAvailable && hasPageGeometry ? PDF_SCREENSHOT_OCR_SCALE : 1;
      const aligned = page.getViewport({ scale: alignScale });
      const { w: aw, h: ah } = previewInnerSize(previewAvail.width, previewAvail.height);
  ...
  const pdfRasterSpace =
    pdfPagePts &&
    pdfPagePts.page === selectedPage &&
    content.boxesAvailable
      ? {
      ...
      : null;
  const space =
    pdfRasterSpace ?? inferPageSpace(selectedPage, content.content.blocks, content.fieldBoxes);
